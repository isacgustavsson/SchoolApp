...
Övningen går ut på att förändra en befintlig kodbas, från en hårt kopplad och sammanhållen kod som är svår att utöka och underhålla, till en lösare kopplad och mer modulär kod som är enklare att utöka och underhålla.

Vi kommer göra detta steg för steg:

1. Nuläget: Stor klump av kod där applikationslogik och UI är starkt kopplade.
2. Skapa en struktur: Dela upp koden i tydliga ansvarsområden. Skapa mappar.
...
Skapa en ny klass SchoolService som hanterar applikationslogiken. Den ska i sin konstruktor ta emot en instans av SchoolDbContext och använda denna för att hämta och spara data. Denna klass ska innehålla dessa metoder:
List<Student> GetAllStudents()
List<Course> GetAllCourses()
void AddStudent(Student student)
void AddCourse(Course course)
void EnrollStudent(int studentId, int courseId)
Skapa en ny klass UI som hanterar användargränssnittet. Den skall i sin konstruktor ta emot en instans av SchoolService och använda denna för att hämta data och utföra funktioner. Metoderna i den kan återspegla de metoder som redan finns i Program.cs. Den bör också ha någon sorts Run()-metod som kör själva while()-loopen.
Skapa mapparna UI, Logic och Infrastructure och flytta UI-klassen till UI-mappen, SchoolService till Logic-mappen samt SchoolDbContext till Infrastructure-mappen.
OBS! Student och Course lägger du i Infrastructure-mappen.
...
Skapa namespaces för klasserna i de olika mapparna. Använd SchoolApp.UI, SchoolApp.Logic och SchoolApp.Infrastructure. Exemelvis namespace SchoolApp.UI; högst upp i UI-klassen.
Se till så att rätt using-statements finns i de olika klasserna:
UI: using SchoolApp.Logic;
Service-klassen: using SchoolApp.Infrastructure;
Program.cs: using SchoolApp.UI; using SchoolApp.Logic; using SchoolApp.Infrastructure; OBS ALLA TRE BEHÖVS HÄR
...
Vi kommer att skapa tre olika separata projekt: Ett för UI, ett för Logic och ett för Infrastructure.
...
Flytta Logic-klasserna till Logic-projektet och ta bort Logic-mappen.
dotnet new classlib -o Infrastructure
Flytta Infrastructure-klassen till SchoolInfrastructure-projektet och ta bort Infrastructure-mappen.
Ta bort Architecture.sln och Architexture.csproj
...
Klicka på SOLUTION EXPLORER i Vs Code och klicka på Open Solution
Högerklicka på Architecture i Solution Explorer och välj Add Existing Project.... Leta reda på de tre .csproj-filerna i de tre projekten och lägg till dem, så att du ser UI, Logic och Infrastructure i Solution Explorer.
...
Högerklicka på UI i Solution Explorer och välj Add Reference.... Klicka på Logic i menyn som kommer upp. Gör det igen och lägg till Infrastructure. Det har nu lagts till referenser i UI.csproj-filen till de andra projekten.
Gör samma sak för Logic, låt den referera till Infrastructure.
Infrastructure är det sista lagret i vår arkitektur så den behöver inte veta något mer.
Prova att köra projektet, om allt gått rätt ska det funka precis som innan. Du kan nu också prova att försöka referera till någon kod i UI eller Logic från Infrastructure-kod, det kommer inte att gå utan ger ett kompileringsfel.

## Steg 4 - Onion Architecture
Det är inte helt bra att vi har våra Students och Courses i datalagret! De borde vara med i kärnan av vårt program, så att vi tex kan testa det utan att bry oss om hur vi lagrar saker. Låt oss strukturera om till en Onion-typ istället. 
För att göra det skall vi vända på beroendet mellan infrastruktur och logic. 
Ta bort beroendet från Logic till Infrastructure genom att ta bort projekt-referensen i Logic.csproj. Lägg sedan istället till ett beroende från Infrastructure till Logic. Nu pekar både UI och Infrastructure till Logic-projektet, men logic-projektet vet inget om vare sig UIn eller Databaser!
Flytta nu Student.cs och Course.cs till Logic-projektet och uppdatera deras namespaces.
Nu kommer du att upptäcka att DbContexten som används i Service-klassen inte längre funkar. Den ligger ju i Infrastructure och dit har vi inte längre någon koppling! Det är nu tricket med Onion-arkitekturen kommer in. I Logic (Det som kallas för Core i onion och clean architecture) måste vi nu definiera repository-interfaces som Service-klassen kan använda sig av i stället. 
(För inspiration till att skriva Repositories: https://deviq.com/design-patterns/repository-pattern)
Exempel på ett generiskt repository med implementation kan du hitta här: Kodexempel på Discord
Kolla i Service-klassen och på alla de platser DbContext-objektet används. Kolla efter vad den används till, vilken data skickas in och vilken data kommer tillbaka. Skapa ett interface med matchande metoder.
...
Här kan det vara bra att stanna upp ett tag och ta sig en funderare. Ligger kod där den borde ligga? Finns det dupliceringar som skulle kunna fixas? Vad har saker och ting för returvärden, vad skickas runt? Var sker validering av data osv? Läs gärna denna artikel och fundera sedan vidare på någon eventuell justering: https://martinfowler.com/bliki/AnemicDomainModel.html 

## Steg 6 - Lägg till ett WebAPI, MVC eller Blazor
Skapa ett nytt projekt av exempelvis WebAPI-typ. Hur kan du göra en endpoint (med minimal API tex) för att lägga till en ny student? Du ska bara behöva använda dig av Infrastruktur-lagret och Logik/Core-lagret och lägga till en student via serviceklassens AddStudent-metod. Du kommer att behöva lägga in DbContexten i DI-containern tillsammans med Repositories och Serviceklass.

## Steg 7 - Vertical Slices eller Event Driven?
